...
namespace AtCoder
{
        ...
        where TValue : INumber<TValue>, IMinMaxValue<TValue>
    {
        ...
        public int AddEdge(int from, int to, TValue cap)
        {
            ...
            Contract.Assert(TValue.Zero <= cap, reason: $"IndexOutOfRange: 0 <= {nameof(cap)}");
            ...
            _g[to].Add(new EdgeInternal(from, fromId, TValue.Zero));
            return m;
        }
        ...
        {
            Contract.Assert((uint)i < (uint)_pos.Count, reason: $"IndexOutOfRange: 0 <= {nameof(i)} && {nameof(i)} < edgeCount");
            Contract.Assert(TValue.Zero <= newFlow && newFlow <= newCap, reason: $"IndexOutOfRange: 0 <= {nameof(newFlow)} && {nameof(newFlow)} <= {nameof(newCap)}");
            var (first, second) = _pos[i];
        ...
        public TValue Flow(int s, int t, TValue flowLimit)
        {
            ...
            void Bfs()
            {
                ...
                while (que.Count > 0)
                {
                    ...
                    {
                        if (EqualityComparer<TValue>.Default.Equals(cap, TValue.Zero) || level[to] >= 0) continue;
                        level[to] = level[v] + 1;
                        if (to == t) return;
                        que.Enqueue(to);
                    }
            ...
            //{
            //    if (v == s) return up;
            //    var res = TValue.Zero(TValue);
            //    for (; iter[v] < _g[v].Count; iter[v]++)
            //    {
            //        EdgeInternal e = _g[v][iter[v]];
            //        if (level[v] <= level[e.To] || EqualityComparer<TValue>.Default.Equals(_g[e.To][e.Rev].Cap, TValue.Zero)) continue;
            //        var up1 = op.Subtract(up, res);
            //        var up2 = _g[e.To][e.Rev].Cap;
            //        var d = Dfs(e.To, op.LessThan(up1, up2) ? up1 : up2);
            //        if (op.LessThanOrEqual(d, TValue.Zero)) continue;
            ...
            DFS: while (stack.Count > 0)
                {
                    ...
                    for (ref var itrv = ref iter[v]; itrv < _g[v].Count; itrv++)
                    {
                        ...
                        if (childOk)
                        {
                            if (level[v] <= level[to] || EqualityComparer<TValue>.Default.Equals(_g[to][rev].Cap, TValue.Zero))
                                continue;

                            var up1 = up - res;
                            var up2 = _g[to][rev].Cap;
                            stack.Push((v, up, res, false));
                            stack.Push((to, (up1 < up2) ? up1 : up2, TValue.Zero, true));
                            goto DFS;
                        }
                        else
                        {
            ...
            while (flow < flowLimit)
            {
                ...
                if (EqualityComparer<TValue>.Default.Equals(f, TValue.Zero)) break;
                flow += f;
            }
            return flow;
        }
        ...
        public bool[] MinCut(int s)
        {
            ...
            while (que.Count > 0)
            {
                ...
                {
                    if (!EqualityComparer<TValue>.Default.Equals(cap, TValue.Zero) && !visited[to])
                    {
                        visited[to] = true;
                        que.Enqueue(to);
                    }
