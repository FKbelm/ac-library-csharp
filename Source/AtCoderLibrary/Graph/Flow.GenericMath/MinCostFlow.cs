...
namespace AtCoder
{
            ...
            where TCost : INumber<TCost>, IMinMaxValue<TCost>
    {
        ...
        public int AddEdge(int from, int to, TCap cap, TCost cost)
        {
            ...
            Contract.Assert(TCap.Zero <= cap, reason: $"IndexOutOfRange: 0 <= {nameof(cap)}");
            Contract.Assert(TCost.Zero <= cost, reason: $"IndexOutOfRange: 0 <= {nameof(cost)}");

            int m = _edges.Count;
            _edges.Add(new Edge(from, to, cap, TCap.Zero, cost));
            return m;
        }
        ...
        {
            return Slope(s, t, TCap.MaxValue);
        }
        ...
        struct TCostComparer : IComparer<TCost>
        {
            [MethodImpl(MethodImplOptions.AggressiveInlining)]
            public int Compare(TCost x, TCost y) => x.CompareTo(y);
        }
        struct SlopeDualRef
        {
            ...
            public bool DualRef()
            {
                ...
                var que = new PriorityQueueOp<TCost, int, TCostComparer>();
                ...
                while (queMin.Count > 0 || que.Count > 0)
                {
                    ...
                    {
                        var e = g.EList[i];
                        if (EqualityComparer<TCap>.Default.Equals(e.Cap, TCap.Zero)) continue;
        ...
        private List<(TCap cap, TCost cost)> Slope(CSR<EdgeInternal> g, int s, int t, TCap flowLimit)
        {
            ...
            TCap flow = TCap.Zero;
            TCost cost = TCost.Zero;
            TCost prevCostPerFlow = TCost.Zero;
            --prevCostPerFlow;
